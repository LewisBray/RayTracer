...
TEST(ray_triangle_intersection) {
    ...
    EXPECT_TRUE(xy_intersection_distance.has_value());
    EXPECT_TRUE(are_equal(xy_intersection_distance.value(), 5.0f));
    ...
    EXPECT_TRUE(!yz_intersection_distance.has_value());
    ...
    EXPECT_TRUE(!zx_intersection_distance.has_value());
    ...
    EXPECT_TRUE(origin_intersection_distance.has_value());
    EXPECT_TRUE(are_equal(origin_intersection_distance.value(), 5.0f));
    ...
    EXPECT_TRUE(!behind_camera_intersection_distance.has_value());
    ...
    EXPECT_TRUE(are_equal(weird_intersection_distance.value(), 3.178055923f));
}
...
TEST(ray_sphere_intersection) {
    ...
    EXPECT_TRUE(twice_intersecting_distances.has_value());
    EXPECT_TRUE(are_equal(twice_intersecting_distances.value().first, -1.0f));
    EXPECT_TRUE(are_equal(twice_intersecting_distances.value().second, 5.0f));
    ...
    EXPECT_TRUE(once_intersecting_distances.has_value());
    EXPECT_TRUE(are_equal(once_intersecting_distances.value().first, once_intersecting_distances.value().second));
    EXPECT_TRUE(are_equal(once_intersecting_distances.value().first, 12.0f));
    ...
    EXPECT_TRUE(internal_intersection_distances.has_value());
    EXPECT_TRUE(are_equal(internal_intersection_distances.value().first, -3.645751311f));
    EXPECT_TRUE(are_equal(internal_intersection_distances.value().second, 1.645751311f));
}

TEST(ray_direction_through_pixel) {
    ...
    for (int x = 0; x < image.width; ++x) {
        for (int y = 0; y < image.height; ++y) {
            ...
            EXPECT_TRUE(are_equal(direction, normalise(pixel_centre - camera.eye)));
        }
    }
}
